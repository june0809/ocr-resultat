...
function contentBands(P, tFrac = 0.35, minLenFrac = 0.05) {
    const H = P.length;
    let min = Infinity;
    let max = -Infinity;
    for (const v of P) {
 ...
 */
async function anchorRows(src, box) {
    const r = (0, source_1.absRect)(box, src.width, src.height);
    const { data, width: W, height: H, channels: C } = await src.grey(r);
    // gradient horizontal moyen par ligne de pixels
    const P0 = new Array(H).fill(0);
    for (let y = 0; y < H; y++) {
        let s = 0;
        for (let x = 0; x < W - 1; x++) {
    ...
    for (let i = 1; i < count; i++) {
        ...
        if (Math.abs(gap - step) / step > MAX_STEP_DEVIATION)
            return null;
    }
